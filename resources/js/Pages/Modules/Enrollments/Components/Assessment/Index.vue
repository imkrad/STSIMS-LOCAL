<template>
   ...
   <Grade @message="checkUpdate()" ref="grade"/>
   <Lock @message="checkUpdate()" ref="lock"/>
</template>
<script>
...
export default {
   components: { simplebar, Lock, Grade },
   data(){
      return {
         lists: [],
   ...
   methods: {
      ...
      checkUpdate(){
         this.selected = this.$page.props.flash.data.data;
         this.lists = this.selected.subjects;
         this.$parent.updateData(this.selected);
      }
   }
}
</script>
